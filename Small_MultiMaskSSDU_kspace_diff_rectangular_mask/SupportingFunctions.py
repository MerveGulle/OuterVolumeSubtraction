...
class OVS_DatasetTrain():
    ...
    def __getitem__(self,index):
        ...
        acc_kspace = self.kspace_data8[...,None]*self.data_consistency_masks[None,:,:,None,:]
        self.x0 = torch.zeros([K,Nx,Ny], dtype=torch.complex64)
        for k in range(K):
            self.x0[k:k+1] = backward(acc_kspace[...,k],self.sense_maps)
        
        return self.x0, self.kspace_data8, self.sense_maps, self.acc_mask, self.data_consistency_masks, self.sub_slc_tf, index
