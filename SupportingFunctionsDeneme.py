...
class OVS_DatasetTest():
    ...
    def __getitem__(self,index):
        ...
        self.diff_kspace      = torch.from_numpy(slice_data['diff_kspace'])
        self.sense_maps       = torch.from_numpy(slice_data['sense_maps'])
        self.composite_image  = torch.from_numpy(slice_data['composite_image'])
        self.acc_mask4        = torch.from_numpy(slice_data['acc_mask4'])
        self.acc_mask8        = torch.from_numpy(slice_data['acc_mask8'])
        self.ovs_mask         = torch.from_numpy(slice_data['ovs_mask'])
        if (self.sense_maps_type == 'sense_maps_mask'):
            self.sense_maps   = torch.from_numpy(slice_data['sense_maps'])*torch.from_numpy(1.0-slice_data['ovs_mask'])[None,:,:,None]
        self.im_tgrappa       = torch.from_numpy(slice_data['im_tgrappa'])
        self.sub_slc_tf       = torch.from_numpy(slice_data['sub_slc_tf'])
        self.x0_diff          = backward(self.diff_kspace, self.sense_maps)
        self.x0               = backward(self.composite_kspace*self.acc_mask8[...,None], self.sense_maps)
        return self.x0, self.x0_diff, self.composite_kspace, self.diff_kspace, self.sense_maps, self.composite_image, self.acc_mask4, self.acc_mask8, self.ovs_mask, self.im_tgrappa, self.sub_slc_tf, index
