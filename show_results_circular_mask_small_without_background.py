...
for sub_counter in range(number_of_subjects):
    ...
    for slc in indices[1,sub_counter][0]:
        ...
        for TF in range(5):
            ...
            slice_data = loadmat("ReferenceImages\subject_"+str(sub)+"_slice_"+str(slc)+"_"+str(TF+1)+".mat")
            im_tgrappa = np.abs(slice_data['im_tgrappa'])
            cg_sense = np.abs(slice_data['cg_sense'])
            ...
            data_range = np.max(np.abs(im_tgrappa_inner))
            ...
            ax = plt.gca()
            NMSE1 = nmse(kfull_Sfull[Nx_min:Nx_max, Ny_min:Ny_max],im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max])
            SSIM1 = ssim(im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max], kfull_Sfull[Nx_min:Nx_max, Ny_min:Ny_max], data_range=data_range)
            ...
            ax = plt.gca()
            NMSE2 = nmse(kdiff_Sfull[Nx_min:Nx_max, Ny_min:Ny_max],im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max])
            SSIM2 = ssim(im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max], kdiff_Sfull[Nx_min:Nx_max, Ny_min:Ny_max], data_range=data_range)
            ...
            ax = plt.gca()
            NMSE3 = nmse(kdiff_Smask[Nx_min:Nx_max, Ny_min:Ny_max],im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max])
            SSIM3 = ssim(im_tgrappa_inner[Nx_min:Nx_max, Ny_min:Ny_max], kdiff_Smask[Nx_min:Nx_max, Ny_min:Ny_max], data_range=data_range)
            ...
            plt.tight_layout()
            plt.savefig("ResultsSmallRectangularCircularMaskWithoutBackground1\subject_"+str(sub)+"_slice_"+str(slc)+"_TF_"+str(time_frame_no)+".png")
            plt.close()
...
n_array = np.arange(1,N_slc+1)
plt.plot(n_array,np.max(SSIM_array,axis=0),'*')
# plt.legend(['kfullSfull','kdiffSfull','kdiffSmask','kdiffSdiff'])
plt.title('SSIM')
plt.tight_layout()
plt.grid()
...
for TF in range(tf-8,tf+8):
     ...
     plt.tight_layout()
     plt.savefig("ResultsSmallCircularMaskWithoutBackground4(sub3slc4tf17)\subject_"+str(sub)+"_slice_"+str(slc)+"_TF_"+str(TF)+".png")
     plt.close()
