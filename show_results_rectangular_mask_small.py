...
for sub_counter in range(number_of_subjects):
    ...
    for slc in indices[1,sub_counter][0]:
        ...
        for TF in range(5):
            ...
            kfull_Sfull = np.abs(slice_data['SSDU_kfull_Sfull'])*(1-ovs_mask)+background*(ovs_mask)
            cg_kfull_Sfull = np.abs(slice_data['cg_sense'])*(1-ovs_mask)+background*(ovs_mask)
            ...
            kdiff_Sfull = np.abs(slice_data['SSDU_kdiff_Sfull'])*(ovs_mask!=1)+background*(ovs_mask)
            cg_kdiff_Sfull = np.abs(slice_data['cg_sense'])*(ovs_mask!=1)+background*(ovs_mask)
            ...
            kdiff_Sdiff = np.abs(slice_data['SSDU_kdiff_Sdiff'])*(ovs_mask!=1)+background*(ovs_mask)
            cg_kdiff_Sdiff = np.abs(slice_data['cg_sense'])*(ovs_mask!=1)+background*(ovs_mask)
            ...
            plt.tight_layout()
            # breakpoint()
            plt.savefig("ResultsSmallRectangularMask3\subject_"+str(sub)+"_slice_"+str(slc)+"_TF_"+str(time_frame_no)+".png")
            plt.close()
        slc_counter +=1