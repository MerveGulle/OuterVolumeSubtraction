import torch
import SupportingFunctionsDeneme as sf
import model
...
### PATHS 
test_data_path = "C:\Codes\p006_OVS\OVS\TestDeneme"
...
denoiser.load_state_dict(torch.load('OVS_multimaskSSDU_100.pt'))  #2
denoiser.eval()

for i, (x0, x0_diff, composite_kspace, diff_kspace, sense_maps, composite_image, acc_mask4, acc_mask8, ovs_mask, im_tgrappa, sub_slc_tf, index) in enumerate(test_loader['test_loader']):
    with torch.no_grad():
        # Forward pass
        x0                     = x0[0,0:1].to(device)                       # [1,Nx,Ny]
        diff_kspace            = composite_kspace[0].to(device)          # [1,Nx,Ny,Nc]
        composite_image        = composite_image[0].to(device).detach().cpu().numpy()          # [1,Nx,Ny]
        acc_mask               = acc_mask8[0].to(device)                 # [Nx,Ny]
        ovs_mask               = ovs_mask[0].to(device).detach().cpu().numpy()             # [Nx,Ny]
        sense_maps             = sense_maps[0,0:1].to(device)               # [1,Nx,Ny,Nc]
        ...
        cg_sense = sf.cgsense(x0,diff_kspace,sense_maps,acc_mask)[0].detach().cpu().numpy()*(1-ovs_mask)
        SSDU = xt[0].detach().cpu().numpy()*(1-ovs_mask)
        im_tgrappa = im_tgrappa.detach().cpu().numpy()
        composite_image = composite_image[0]
        ...
        print(slc)
        print(tf)
        #filename = "C:\Codes\p006_OVS\OVS\Deneme\kfull_Sfull\subject_"+str(sub)+"_slice_"+str(slc)+"_TF_"+str(tf)+".mat" #3
        filename = "C:\Codes\p006_OVS\OVS\Deneme\slice_info\subject_"+str(sub)+"_slice_"+str(slc)+"_TF_"+str(tf)+".mat" #3

        #datadir = {"SSDU_kfull_Sfull": SSDU, #4
        #           "cg_sense": cg_sense}
        datadir = {"composite_image": composite_image,
                   "im_tgrappa": im_tgrappa,
                   "ovs_mask": ovs_mask}
